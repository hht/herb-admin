...
import { Drawer, Loading } from "tdesign-react"
...
import { getConsultationDetail } from "~/services/app-user-qtn"
import {
  getAppOrderDetail,
...
const buildServiceLabel = (value: unknown, index: number) => {
  ...
  if (text) return text
  const cnNumber = ["一", "二", "三", "四", "五", "六", "七", "八", "九", "十"][index] ?? `${index + 1}`
  return `服务${cnNumber}`
}
...
export const OrderDetailDrawer = ({
  ...
  const batchNo =
    order?.userAnswerId === null || typeof order?.userAnswerId === "undefined"
      ? undefined
      : String(order.userAnswerId)

  const { data: consultationDetail, loading: consultationLoading } = useRequest(
    () => (visible && batchNo ? getConsultationDetail(batchNo) : Promise.resolve(null)),
    { refreshDeps: [visible ? batchNo ?? "" : "hidden"] }
  )

  const doctorDiagnosis = getText(
    consultationDetail?.consultation?.doctorMsg,
    consultationDetail?.consultation?.adviceMsg,
    record.doctorMsg,
    ...
    >
      <div className="flex h-full flex-col bg-white">
        ...
        <div className="min-h-0 flex-1 overflow-hidden">
          <Loading loading={orderLoading || consultationLoading} className="h-full">
                  ...
                  <div className="mt-6 space-y-8">
                    ...
                    {serviceItems.length ? (
                      serviceItems.map((service) => (
                        <div key={`${service.label}-${service.name}`} className="space-y-2">
                          <div className="text-[14px] leading-[22px] text-[rgba(0,0,0,0.4)]">
                            {service.label}
                          </div>
                          <div className="space-y-2">
