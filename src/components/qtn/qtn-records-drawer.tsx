...
export const QtnRecordsDrawer = ({
  ...
  const { data, loading, error: listError } = useRequest(
    ...
    {
      refreshDeps: [visible, userId, groupId, refreshNonce, JSON.stringify(page)],
    }
  )
  ...
  const { runAsync: runDelete, loading: deleting } = useRequest(
    (consultationId: number) => deleteConsultation(consultationId),
    { manual: true }
  )
  ...
  const filteredRecords = useMemo(() => {
    if (patientFilter === "全部病患") return records
    return records.filter((item) => item.name === patientFilter)
  }, [records, patientFilter])
  ...
  const handleDelete = (record: QtnRecord) => {
    ...
    if (!record.consultationId) {
      MessagePlugin.warning("缺少问诊记录ID，无法删除")
      return
    }
    const dialog = DialogPlugin.confirm({
      header: "确认删除",
      body: `确定删除问诊记录${label}吗？`,
      confirmBtn: { content: "删除", loading: deleting },
      cancelBtn: "取消",
      onConfirm: async () => {
        await runDelete(record.consultationId as number)
        MessagePlugin.success("问诊记录已删除")
        dialog.hide()
        setState((state) => ({
          selected: state.selected.filter(
            (item) => getRecordKey(item) !== getRecordKey(record)
          ),
        }))
        setRefreshNonce((prev) => prev + 1)
        if (view !== "list") {
          setState({ view: "list", activeRecord: null, detailData: null })
        }
      },
      onClose: () => dialog.hide(),
    })
  ...
  const baseColumns = [
    ...
    { colKey: "name", title: "病患姓名", width: 150 },
    {
      colKey: "createTime",
      title: "创建日期",
      width: 170,
  ...
  const listView = (
    <div className="flex-1 min-h-0 overflow-y-auto px-12 py-6">
      {!userId && !groupId ? (
        <div className="mb-4 rounded border border-border bg-white p-4 text-sm text-neutral-950/60">
          请选择群聊后查看问诊记录
        </div>
      ) : null}
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Select
          value={patientFilter}
          onChange={(value) => setState({ patientFilter: String(value) })}
          options={patientOptions.map((item) => ({ label: item, value: item }))}
          className="w-[180px]"
        />
      </div>
  ...
  const headerTitle = useMemo(() => {
    ...
    const resolvedName = userName ?? records[0]?.name
